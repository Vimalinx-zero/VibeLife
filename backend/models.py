...
from sqlalchemy import Column, Integer, String, Boolean, Float, DateTime, ForeignKey, JSON, Text, UniqueConstraint
...
class Question(Base):
    ...
    # 选择题选项（仅选择题使用）
    options = Column(JSON, nullable=True) # [{key: "A", content: "...", is_correct: true}]
...
class StudySession(Base):
    ...
    created_at = Column(String, default=lambda: datetime.datetime.utcnow().isoformat())  # ISO 8601
...
class WorkbenchMistake(Base):
    ...
    created_at = Column(String, default=lambda: datetime.datetime.utcnow().isoformat())  # 创建时间
...
class CardReview(Base):
    ...
    review_time = Column(String, default=lambda: datetime.datetime.utcnow().isoformat())  # 复习时间
...
class Collection(Base):
    ...
    color = Column(String, default="blue")  # 显示颜色 (blue, green, red, yellow, purple)
...
class CollectionCard(Base):
    ...
    added_at = Column(String, default=lambda: datetime.datetime.utcnow().isoformat())  # 添加时间
...
class FavoriteQuestion(Base):
    ...
    added_at = Column(String, default=lambda: datetime.datetime.utcnow().isoformat())  # 收藏时间
